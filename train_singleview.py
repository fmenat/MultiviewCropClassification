...
def main_run(config_file):
    ...
    train_data_target = labels_tr.astype(int).flatten()
    ...
    for r in range(runs):
        if os.path.isfile(f"{output_dir_folder}/pred/{data_name}/test/Input/out_run-{r:02d}.csv") and not OVERWRITE:
        ...
        if val_size!= 0:
            mask_train = np.random.rand(data_views_tr.get_n_total()) <= (1-val_size)
        ...
        start_aux = time.time()
        method, trainer = InputFusion_train(train_data, val_data=val_data, run_id=r,method_name=method_name ,
        ...
        data_save_tr.save(f"{output_dir_folder}/pred/{data_name}/train/Input", ind_views=True,xarray=False)
        ...
        data_save_te.save(f"{output_dir_folder}/pred/{data_name}/test/Input", ind_views=True,xarray=False)
